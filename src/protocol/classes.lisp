...
(defclass amqp-basic-class-properties ()
 (
 (content-type :type amqp-shortstr :initarg :content-type :accessor amqp-property-content-type)
 (content-encoding :type amqp-shortstr :initarg :content-encoding :accessor amqp-property-content-encoding)
 (headers :type amqp-table :initarg :headers :accessor amqp-property-headers)
 (delivery-mode :type amqp-octet :initarg :delivery-mode :accessor amqp-property-delivery-mode)
 (priority :type amqp-octet :initarg :priority :accessor amqp-property-priority)
 (correlation-id :type amqp-shortstr :initarg :correlation-id :accessor amqp-property-correlation-id)
 (reply-to :type amqp-shortstr :initarg :reply-to :accessor amqp-property-reply-to)
 (expiration :type amqp-shortstr :initarg :expiration :accessor amqp-property-expiration)
 (message-id :type amqp-shortstr :initarg :message-id :accessor amqp-property-message-id)
 (timestamp :type amqp-timestamp :initarg :timestamp :accessor amqp-property-timestamp)
 (type :type amqp-shortstr :initarg :type :accessor amqp-property-type)
 (user-id :type amqp-shortstr :initarg :user-id :accessor amqp-property-user-id)
 (app-id :type amqp-shortstr :initarg :app-id :accessor amqp-property-app-id)
 (cluster-id :type amqp-shortstr :initarg :cluster-id :accessor amqp-property-cluster-id)
))
...
(defmethod amqp-class-properties-decoder ((class (eql 'amqp-basic-class-properties)) ibuffer)
  (let* ((flags 0)
         (flagword-index 0)
         (initargs
           (progn
            ...
            (collectors:with-appender-output (add-initarg)
             ...
             (unless (zerop (logand flags +flag-content-encoding+))
               (add-initarg :content-encoding (amqp-shortstr-decoder ibuffer)))
             (unless (zerop (logand flags +flag-headers+))
               (add-initarg :headers (amqp-table-decoder ibuffer)))
             (unless (zerop (logand flags +flag-delivery-mode+))
               (add-initarg :delivery-mode (amqp-octet-decoder ibuffer)))
             (unless (zerop (logand flags +flag-priority+))
               (add-initarg :priority (amqp-octet-decoder ibuffer)))
             (unless (zerop (logand flags +flag-correlation-id+))
               (add-initarg :correlation-id (amqp-shortstr-decoder ibuffer)))
             (unless (zerop (logand flags +flag-reply-to+))
               (add-initarg :reply-to (amqp-shortstr-decoder ibuffer)))
             (unless (zerop (logand flags +flag-expiration+))
               (add-initarg :expiration (amqp-shortstr-decoder ibuffer)))
             (unless (zerop (logand flags +flag-message-id+))
               (add-initarg :message-id (amqp-shortstr-decoder ibuffer)))
             (unless (zerop (logand flags +flag-timestamp+))
               (add-initarg :timestamp (amqp-timestamp-decoder ibuffer)))
             (unless (zerop (logand flags +flag-type+))
               (add-initarg :type (amqp-shortstr-decoder ibuffer)))
             (unless (zerop (logand flags +flag-user-id+))
               (add-initarg :user-id (amqp-shortstr-decoder ibuffer)))
             (unless (zerop (logand flags +flag-app-id+))
               (add-initarg :app-id (amqp-shortstr-decoder ibuffer)))
             (unless (zerop (logand flags +flag-cluster-id+))
               (add-initarg :cluster-id (amqp-shortstr-decoder ibuffer)))
             ))))
